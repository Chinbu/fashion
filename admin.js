        ...
        product.links.forEach((link, i) => {
            ...
            group.innerHTML = `
                <div style="display: flex; gap: 10px; align-items: center; margin: 10px 0;">
                    ...
                    <span class="platform-badge" id="folderPlatformBadge${folderLinkCounter}" style="background: ${getPlatformColor(platform)}; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; white-space: nowrap;">${getPlatformIcon(platform)} ${platform}</span>
...
async function loadProductsList() {
    ...
    products.forEach(product => {
        ...
        links.forEach(link => {
            const platform = link.platform || detectPlatformFromUrl(link.url) || 'Other';
            platformBadges += `<span style="background: ${getPlatformColor(platform)}; color: white; padding: 2px 10px; border-radius: 10px; font-size: 11px; margin: 2px;">${getPlatformIcon(platform)} ${platform}</span>`;
        });
            ...
            links.forEach((link, index) => {
                ...
                group.innerHTML = `
                    <div style="display: flex; gap: 10px; align-items: center; margin: 10px 0;">
                        ...
                        <span class="platform-badge" id="platformBadge${linkCounter}" style="background: ${getPlatformColor(platform)}; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; white-space: nowrap;">${getPlatformIcon(platform)} ${platform}</span>
